"""
Collect metrics from results/ folder and export to metrics_results.xlsx.

Structure: results/reactor_<model>/<single_subfolder>/results.json
Models:    csdi, mambats, ncde, probabilistic, tft, timexer  (timellm excluded)

Per-batch metrics come from per_experiment_metrics (overall/spike/non_spike).
Output: results_summary.json  +  metrics_results.xlsx
"""
...
WANTED      = {"csdi", "mambats", "ncde", "probabilistic", "tft", "timexer"}
...
def main():
    ...
    print(f"Written: {out_dir / 'results_summary.json'}")
    for model, batches in summary.items():
        print(f"  {model}: {list(batches.keys())}")

    rows = flatten(summary)
    if rows:
        export(pd.DataFrame(rows), out_dir / "metrics_results.xlsx")
