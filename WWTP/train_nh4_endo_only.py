"""
train_nh4_endo_only.py – Forecast T1_NH4 using only its own history.

No exogenous variables. No residual anchor. Pure endogenous self-attention
over patches of the NH4 time series.

Compare against:
  train_nh4_exo_o2.py   → same architecture but with 9 exo vars via TCN + cross-attn
  vanilla_nh4_exo_o2.py → vanilla exo (1 token per var, no TCN)

Architecture
------------
  Endogenous: patch → linear projection → positional embedding
  Encoder:    global token + patches → self-attention → FFN  (no cross-attn)
  Head:       LayerNorm → Linear(d_model → horizon)  — absolute values, no anchor

Usage
-----
    python train_nh4_endo_only.py
    python train_nh4_endo_only.py --epochs 50 --d_model 128 --n_heads 8 --n_layers 3 --d_ff 512
"""
...
class WWTPDataset(Dataset):
    def __init__(self, endo, context_len, horizon, stride=1):
...
def build_dataloaders(data_path, context_len, horizon, batch_size,
    ...
    endo_tr = sc_endo.transform(endo[:n_train])
    endo_va = sc_endo.transform(endo[n_train:n_train+n_val])
    endo_te = sc_endo.transform(endo[n_train+n_val:])

    kw = dict(context_len=context_len, horizon=horizon, stride=stride)
    nw = min(4, __import__('os').cpu_count() or 1)
    train_dl = DataLoader(WWTPDataset(endo_tr, **kw), batch_size=batch_size,
                          shuffle=True,  num_workers=nw, pin_memory=True, drop_last=True)
    val_dl   = DataLoader(WWTPDataset(endo_va, **kw), batch_size=batch_size,
                          shuffle=False, num_workers=nw, pin_memory=True)
    test_dl  = DataLoader(WWTPDataset(endo_te, **kw), batch_size=batch_size,
                          shuffle=False, num_workers=nw, pin_memory=True)
    ...
    return train_dl, val_dl, test_dl, sc_endo
...
class EncoderLayer(nn.Module):
    """Self-attention only — no cross-attention (no exogenous variables)."""
    ...
    def forward(self, patches, global_tok):
...
class NH4EndoModel(nn.Module):
    """
    Pure endogenous NH4 forecaster.
    No exogenous variables. No residual anchor — head predicts absolute values.
    """
    ...
    def forward(self, x_endo):
        ...
        for layer in self.layers:
            patches, glob = layer(patches, glob)
        return self.head(glob)                             # absolute, no anchor
...
def train_epoch(model, loader, optimizer, criterion, device):
    model.train()
    total, n = 0.0, 0
    for x_endo, y in loader:
        x_endo, y = x_endo.to(device), y.to(device)
        optimizer.zero_grad()
        loss = criterion(model(x_endo), y)
        loss.backward()
...
def eval_epoch(model, loader, criterion, device):
    ...
    for x_endo, y in loader:
        x_endo, y = x_endo.to(device), y.to(device)
        total += criterion(model(x_endo), y).item() * x_endo.size(0)
        n += x_endo.size(0)
    return total / n
...
def collect_predictions(model, loader, device):
    ...
    preds, targets = [], []
    for x_endo, y in loader:
        preds  .append(model(x_endo.to(device)).cpu().numpy())
...
def plot_horizon(model, sc_endo, data_path, context_len, horizon,
    ...
    for s in starts:
        ...
        with torch.no_grad():
            pred = model(x_e).squeeze(0).cpu().numpy()    # [1, H]
        pred_orig = sc_endo.inverse_transform(pred.T).T   # [1, H]
        windows.append({
            'window_idx':  int(s),
    ...
    fig.suptitle(f'NH4 endogenous-only forecast — no exogenous, no residual anchor\n'
...
def main(args):
    ...
    print(f"Endogenous : {ENDO_COLS}  (no exogenous, no residual anchor)")

    print("\n── Loading data ──────────────────────────────────────────────")
    train_dl, val_dl, test_dl, sc_endo = build_dataloaders(
    ...
    print(f"\n  T1_NH4  MAE={mae:.4f}  RMSE={rmse:.4f}")
    ...
    ax.set_title('NH4 endogenous-only — training history')
    ax.legend(); ax.grid(alpha=0.3); fig.tight_layout()
    fig.savefig(out_dir / 'training_loss.png', dpi=150); plt.close(fig)

    plot_horizon(model, sc_endo, args.data_path,
...
if __name__ == '__main__':
    ...
    p.add_argument('--out_dir',     default='results3/nh4_endo_only_no_anchor_short')
    main(p.parse_args())
