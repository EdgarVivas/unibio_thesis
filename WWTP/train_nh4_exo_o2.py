"""
train_nh4_exo_o2.py – Forecast only T1_NH4, with T1_O2 treated as an
exogenous variable alongside the 8 real sensors.

Purpose
-------
Isolates whether the exogenous cross-attention pathway is powerful enough to
leverage the O2 signal on its own — without any explicit cross-variate coupling
between endogenous variables.

Compare against:
  train.py            → joint O2+NH4 with cross-variate patches
  train_channel_indep → joint O2+NH4 channel-independent
  vanilla_timexer.py  → joint O2+NH4 vanilla TimeXer

Architecture
------------
  c_endo = 1  (T1_NH4 only)
  c_exo  = 9  (8 real sensors + T1_O2)
  No cross-variate step (only one endogenous variable, nothing to couple)
  Exogenous: patch → shared TCN → variable embedding → cross-attention

Usage
-----
    python train_nh4_exo_o2.py
    python train_nh4_exo_o2.py --epochs 50 --d_model 128 --n_heads 8 --n_layers 3 --d_ff 512
"""
...
EXO_COLS  = [
    ...
    'T1_O2',          # O2 as exogenous — goes through TCN + cross-attention
]
...
class EncoderLayer(nn.Module):
    """Single encoder layer — no cross-variate (only one endogenous variable)."""
...
class NH4Model(nn.Module):
    """
    TimeXer for single target (NH4), with O2 treated as one of 9 exogenous
    variables processed through patch embedding + shared TCN + variable embedding.
    """
    def __init__(self, c_exo, context_len, horizon,
        ...
        # Exogenous (8 sensors + O2) — same TCN pipeline as main model
    ...
    def forward(self, x_endo, x_exo):
        ...
        B = x_endo.shape[0]
        exo_kv  = self._encode_exo(x_exo)                        # [B, 9*N_p, d]
...
# ─────────────────────────────────────────────────────────────────────────────
# Horizon plot — same seed/split as other files for direct comparison
# ─────────────────────────────────────────────────────────────────────────────
...
def plot_horizon(model, sc_endo, sc_exo, data_path, context_len, horizon,
    ...
    for s in starts:
        ...
        with torch.no_grad():
            pred = model(x_e, x_x).squeeze(0).cpu().numpy()      # [1, H]
    ...
    fig.suptitle(f'NH4-only forecast — O2 as exogenous variable\n'
...
def main(args):
    ...
    model = NH4Model(
        ...
        n_layers=args.n_layers, d_ff=args.d_ff, tcn_levels=args.tcn_levels,
        tcn_kernel=args.tcn_kernel, dropout=args.dropout,
    ).to(device)
    ...
    N, C, H = preds.shape
    p_orig = sc_endo.inverse_transform(preds[:,0,:])   # [N, H]
    ...
    ax.set_title('NH4-only (O2 as exogenous) — training history')
...
if __name__ == '__main__':
    ...
    p.add_argument('--out_dir',     default='results3/test2')
    main(p.parse_args())
